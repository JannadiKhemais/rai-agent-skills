...
| `Query error` / `Unreachable` on `model.define()` chain | `model.define()` references aggregation-computed property in arithmetic (even transitively) | `model.define()` cannot chain on aggregation results at all — not just subtypes. Define the agg property but compute downstream values at query time with pandas |
| `Query error` poisons unrelated queries | Broken `model.define()` (agg-dependent or OR in subtype) in model | Remove the broken definition; it affects ALL queries on the parent entity |
| `Query error` on subtype with OR (`\|`) | Subtype uses `(a == "x") \| (a == "y")` in `where()` | Split into multiple `model.define()` calls — one per OR branch |
| `[Unknown Concept]` on `model.Property()` | Double braces `{{model.X}}` in f-string outputs literal text | Use single braces: `f"{model.Entity} has prop {model.ValueType}"` |
| `AttributeError` on non-existent model function | Non-existent function (e.g., `model.parse_int`, `model.cast`) | Valid model methods: `model.define`, `model.where`, `model.select`, `model.require`, `model.not_`, `model.Concept`, `model.Property`, `model.Relationship`. Standalone: `count/sum/avg/min/max`, `Float/Integer/String/Boolean` |
| `bad operand type for unary ~: 'Chain'` | Used `~` to negate a boolean relationship in `model.where()` | No direct negation for boolean relationships; use two-query pandas subtraction pattern |
...
| `TyperError` / `Type errors detected during type inference` when querying subtypes | Accessing properties directly on subtype concept (`model.FlopMovie.title`) | Bind subtype to parent with `model.Subtype(model.Parent)` in `where()`, then access all properties via `model.Parent.prop` in `select()` |
| `TyperError` when selecting a boolean relationship as column | Boolean relationship (e.g., `model.Entity.was_clicked.alias("col")`) placed in `select()` | Relationships are filter-only — use in `where()` only. To project as column: query all entities, query flagged subset, then `df["flag"] = df["id"].isin(flagged_ids["id"])` |
...
| `Unreachable` on cross-entity `model.define()` | Dot-chain navigation (`model.EntityA.rel.prop`) in `model.define()` arithmetic | Use explicit join: `model.EntityB.prop` in arithmetic + `model.EntityA.rel(model.EntityB)` in `.where()` |
